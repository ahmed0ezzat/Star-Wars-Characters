...
function CharacterModal({ character, onClose }: CharacterModalProps) {
  ...
  const homeworldSection = () => (
    <div
      className="bg-gradient-to-r from-gray-800/50 to-gray-800/30 p-4 rounded-lg border border-gray-700/50"
      data-testid="modal-character-created"
    >
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in" data-testid="character-modal">
      ...
      <div className="relative w-full max-w-md rounded-2xl bg-gradient-to-br from-gray-900 to-gray-800 shadow-2xl shadow-yellow-500/20 animate-scale-in border border-gray-700 overflow-hidden">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-10 p-2 rounded-full bg-gray-800/80 hover:bg-gray-700/90 transition-all text-gray-300 hover:text-white"
          aria-label="Close"
        >
